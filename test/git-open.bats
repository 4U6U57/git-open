...
@test "sshconfig: no action on inproper alias casing" {
  create_ssh_sandbox
  git remote set-url origin "git@Basic:user/repo.git"
  run ../git-open
  assert_output "https://Basic/user/repo"
}

@test "sshconfig: with git user" {
  create_ssh_sandbox
...
@test "sshconfig: unusual casing on Host/HostName keywords" {
  create_ssh_sandbox
...
@test "sshconfig: multitarget host with substitution" {
  create_ssh_sandbox
  for i in $(seq 1 3); do
...
@test "sshconfig: host wildcard ?" {
  ...
  run ../git-open
  refute_output "https://one.com/user/repo"
}
...
# Sample SSH config (written to ~/.ssh/config)
ssh_testdata="
